...
Size ProcGlobalShmemSize(void) {
  ...
  size =
      add_size(size, mul_size(TotalProcs, sizeof(*ProcGlobal->subxidStates)));
...
void InitProcGlobal(void) {
  ...
  ProcGlobal =
      (PROC_HDR *)ShmemInitStruct("Proc Header", sizeof(PROC_HDR), &found);
  Assert(!found);
  ...
  ProcGlobal->subxidStates = (XidCacheStatus *)ShmemAlloc(
      TotalProcs * sizeof(*ProcGlobal->subxidStates));
  MemSet(ProcGlobal->subxidStates, 0,
         TotalProcs * sizeof(*ProcGlobal->subxidStates));
  ProcGlobal->statusFlags =
      (uint8 *)ShmemAlloc(TotalProcs * sizeof(*ProcGlobal->statusFlags));
  MemSet(ProcGlobal->statusFlags, 0,
         TotalProcs * sizeof(*ProcGlobal->statusFlags));
  ...
  for (i = 0; i < TotalProcs; i++) {
    ...
    if (i < MaxConnections) {
      ...
      proc->procgloballist = &ProcGlobal->autovacFreeProcs;
                       ...
                       NUM_SPECIAL_WORKER_PROCS + max_worker_processes) {
...
void InitProcess(void) {
   ...
   */
  {
    elog(WARNING, "# 309: you already exist (MyProc=%p), skipping re-init",
         MyProc);
    return;
  }
#else
  ...
  if (!dlist_is_empty(procgloballist)) {
    ...
    if (AmWalSenderProcess())
      ereport(FATAL, (errcode(ERRCODE_TOO_MANY_CONNECTIONS),
                      errmsg("number of requested standby connections exceeds "
  ...
  Assert(pg_atomic_read_u32(&MyProc->procArrayGroupNext) ==
         INVALID_PROC_NUMBER);
...
ProcWaitStatus ProcSleep(LOCALLOCK *locallock, LockMethod lockMethodTable,
  ...
  do {
    if (InHotStandby) {
      ...
      if (maybe_log_conflict) {
        ...
        if (TimestampDifferenceExceeds(standbyWaitStart, now,
          ...
          vxids =
              GetLockConflicts(&locallock->tag.lock, AccessExclusiveLock, &cnt);
           ...
           */
          LogRecoveryConflict(PROCSIG_RECOVERY_CONFLICT_LOCK, standbyWaitStart,
                              now, cnt > 0 ? vxids : NULL, true);
    ...
    if (deadlock_state == DS_BLOCKED_BY_AUTOVACUUM && allow_autovacuum_cancel) {
      ...
      if ((statusFlags & PROC_IS_AUTOVACUUM) &&
        ...
        if (message_level_is_interesting(DEBUG1)) {
          ...
          appendStringInfo(&logbuf, "Process %d waits for %s on %s.", MyProcPid,
                           GetLockmodeName(lockmethod_copy, lockmode),
                           locktagbuf.data);

          ereport(DEBUG1,
                  (errmsg_internal(
                       "sending cancel to blocking autovacuum PID %d", pid),
    ...
    if (log_lock_waits && deadlock_state != DS_NOT_YET_CHECKED) {
      ...
      modename =
          GetLockmodeName(locallock->tag.lock.locktag_lockmethodid, lockmode);
      ...
      dlist_foreach(proc_iter, &lock->procLocks) {
        curproclock = dlist_container(PROCLOCK, lockLink, proc_iter.cur);
      ...
      if (deadlock_state == DS_SOFT_DEADLOCK)
        ereport(LOG, (errmsg("process %d avoided deadlock for %s on %s by "
                             "rearranging queue order after %ld.%03d ms",
         ...
         * events get logged.
         */
        ereport(LOG, (errmsg("process %d detected deadlock while waiting for "
                             "%s on %s after %ld.%03d ms",
                             MyProcPid, modename, buf.data, msecs, usecs),
                      (errdetail_log_plural(
                          "Process holding the lock: %s. Wait queue: %s.",
                          "Processes holding the lock: %s. Wait queue: %s.",
      ...
      if (myWaitStatus == PROC_WAIT_STATUS_WAITING)
        ...
        ereport(LOG, (errmsg("process %d acquired %s on %s after %ld.%03d ms",
                             MyProcPid, modename, buf.data, msecs, usecs)));
      else {
  ...
  if (InHotStandby && logged_recovery_conflict)
    LogRecoveryConflict(PROCSIG_RECOVERY_CONFLICT_LOCK, standbyWaitStart,
                        GetCurrentTimestamp(), NULL, false);
