 ...
 */
void PglMobileResetShmemState(void) {
  fprintf(
      stderr,
      "[PGL_MOBILE] PglMobileResetShmemState: Resetting shared memory state\n");
  fprintf(
      stderr,
      "[PGL_MOBILE]   ShmemSegHdr=%p ShmemBase=%p ShmemEnd=%p ShmemIndex=%p\n",
...
void InitShmemAllocation(void) {
   ...
   */
  elog(DEBUG1, "InitShmemAllocation: pre-alloc totalsize=%zu freeoffset=%zu",
       shmhdr->totalsize, shmhdr->freeoffset);
  ShmemLock = (slock_t *)ShmemAllocUnlocked(sizeof(slock_t));
  elog(DEBUG1, "InitShmemAllocation: post-alloc freeoffset=%zu ShmemLock=%p",
       shmhdr->freeoffset, (void *)ShmemLock);
...
void *ShmemAllocUnlocked(Size size) {
  ...
  newStart = ShmemSegHdr->freeoffset;
  elog(DEBUG1, "ShmemAllocUnlocked: req=%zu start=%zu totalsize=%zu", size,
       newStart, ShmemSegHdr->totalsize);
                    ...
                    int hash_flags)   /* info about infoP */
{
  ...
  location =
      ShmemInitStruct(name, hash_get_shared_size(infoP, hash_flags), &found);
...
void *ShmemInitStruct(const char *name, Size size, bool *foundPtr) {
  ...
  /* look it up in the shmem index */
  result =
      (ShmemIndexEnt *)hash_search(ShmemIndex, name, HASH_ENTER_NULL, foundPtr);
